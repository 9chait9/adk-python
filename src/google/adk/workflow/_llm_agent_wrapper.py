...
def prepare_llm_agent_context(agent: Any, ctx: Context) -> Context:
  ...
  # For single_turn agents, ensure session events are deeply isolated
  # and start clean to prevent input visibility issues from prior runs.
  ic.session = ic.session.model_copy(deep=True) # <--- deep copy
  ic.session.events = [] # <--- clear events
  return agent_ctx
